...
Menu.Tokens = {
    ...
    Danger      = { r = 255, g = 70,  b = 75 },
    Success     = { r = 60,  g = 220, b = 145 }
}
Menu.Colors = Menu.Tokens
...
function Menu.GetActualHeight()
    ...
    local scale = Menu.Scale or 1.0
    local bannerH = (Menu.Banner.enabled and not Menu.Settings.DisableAnimatedBanners) and (Menu.Banner.height * scale) or p.headerHeight
...
function Menu.ExpApproach(current, target, speed)
    if Menu.Settings and Menu.Settings.ReduceAnimations then return target end
...
function Menu.DrawHeader()
    ...
    local bannerTexture = Menu.bannerTexture
    
    -- Corregir banner de submenús
    if Menu.OpenedCategory then
        local cat = SafeTable(Menu.Categories)[Menu.OpenedCategory]
        if cat and cat.bannerTexture then bannerTexture = cat.bannerTexture end
    end

    local bannerH = (Menu.Banner.enabled and not Menu.Settings.DisableAnimatedBanners) and (Menu.Banner.height * (Menu.Scale or 1.0)) or h
    if Menu.Banner.enabled and not Menu.Settings.DisableAnimatedBanners and bannerTexture and bannerTexture>0 and Susano.DrawImage then
        Susano.DrawImage(bannerTexture, x, y, w, bannerH, 1,1,1,(Menu.RenderAlpha or 1.0),0)
    else
        Menu.DrawRoundedRect(x, y, w, h, 0,0,0, Menu.Colors.BgMain.a/255.0, Menu.Settings.HighContrast and 0 or p.headerRadius)
...
function Menu.DrawScrollbar(x, startY, visibleHeight, selectedIndex, totalItems, isMainMenu, menuWidth)
    if totalItems <= Menu.ItemsPerPage then return end
    local p = Menu.GetScaledPosition()
    local scale = Menu.Scale or 1.0
    local width = menuWidth or p.width
    local sbW = p.scrollbarWidth
    local sbX = x + width - sbW - 4*scale
    local sbY = startY
    local sbH = visibleHeight

    -- Rail simple
    Menu.DrawRect(sbX, sbY, sbW, sbH, 0, 0, 0, 0.3)
    
    local thumbH = math.max(10*scale, sbH * (Menu.ItemsPerPage / totalItems))
    local progress = (selectedIndex - 1) / math.max(1, totalItems - 1)
    local targetY = sbY + progress * (sbH - thumbH)
    
    local stateName = isMainMenu and "SmoothScrollCatY" or "SmoothScrollItemY"
    Menu[stateName] = Menu.ExpApproach(Menu[stateName] or targetY, targetY, 12.0)
    local thumbY = Menu[stateName]
    
    Menu.DrawRect(sbX, thumbY, sbW, thumbH, Menu.Colors.Accent.r/255.0, Menu.Colors.Accent.g/255.0, Menu.Colors.Accent.b/255.0, 0.8)
end
...
function Menu.DrawFooter()
    ...
    local help = "Navegar: ↑↓  |  Cambiar: ←→  |  Seleccionar: Intro"
    if item and item.description then help = item.description end
    
    local fs = 11
    Menu.DrawText(x+15, footerY+h/2-fs/2, help, fs, Menu.Colors.TextDim.r/255.0, Menu.Colors.TextDim.g/255.0, Menu.Colors.TextDim.b/255.0, 255)
    ...
    if page ~= "" then
        ...
        Menu.DrawText(x+w-vw-pw-30, footerY+h/2-fs/2, page, fs, Menu.Colors.Text.r/255.0, Menu.Colors.Text.g/255.0, Menu.Colors.Text.b/255.0, 255)
    end
end
...
function Menu.DrawBackground()
    ...
    local opacity = Menu.Settings.HighContrast and 1.0 or (Menu.Settings.BgOpacity or Menu.Colors.BgMain.a/255.0)
    
    -- Fondo negro con opacidad ajustable
    Menu.DrawRoundedRect(x, y, w, actualHeight, 0,0,0, opacity, Menu.Settings.HighContrast and 0 or p.headerRadius)
    
    if Menu.Settings.ShowSnowflakes and not Menu.Settings.DisableParticles then
        local dt = (Menu.FrameDelta or 0.016) * 60.0 -- Normalizar a 60 FPS
        for _, part in ipairs(Menu.Particles) do
            part.y = part.y + part.speedY * dt
            part.x = part.x + part.speedX * dt
            ...
            if part.size >= 2 and Menu.Settings.IntensityGlows then
...
function Menu.GetAccessibilityItems()
    return {
        { name = "Accesibilidad Visual", isSeparator = true },
        { name = "Reducir animaciones", type = "toggle", value = Menu.Settings.ReduceAnimations, description = "Desactiva transiciones suaves para mayor rapidez.", setting = "ReduceAnimations", onClick = function(v) Menu.Settings.ReduceAnimations = v end },
        { name = "Desactivar partículas", type = "toggle", value = Menu.Settings.DisableParticles, description = "Elimina la nieve y efectos de fondo.", setting = "DisableParticles", onClick = function(v) Menu.Settings.DisableParticles = v end },
        { name = "Escala de texto", type = "slider", min = 0.8, max = 1.6, step = 0.1, value = Menu.Settings.TextScale, description = "Ajusta el tamaño de la fuente de la interfaz.", setting = "TextScale", onClick = function(v) Menu.Settings.TextScale = v end },
        { name = "Opacidad fondo", type = "slider", min = 0.6, max = 0.95, step = 0.05, value = Menu.Settings.BgOpacity, description = "Ajusta la transparencia del panel principal.", setting = "BgOpacity", onClick = function(v) Menu.Settings.BgOpacity = v end },
        { name = "Contraste alto", type = "toggle", value = Menu.Settings.HighContrast, description = "Fondo negro sólido y bordes rectos.", setting = "HighContrast", onClick = function(v) Menu.Settings.HighContrast = v end },
        { name = "Ocultar banners", type = "toggle", value = Menu.Settings.DisableAnimatedBanners, description = "Sustituye banners por un logo estático.", setting = "DisableAnimatedBanners", onClick = function(v) Menu.Settings.DisableAnimatedBanners = v end },
        { name = "Repetición de teclas", type = "toggle", value = Menu.Settings.KeyRepeat, description = "Navegación rápida manteniendo presionadas las teclas.", setting = "KeyRepeat", onClick = function(v) Menu.Settings.KeyRepeat = v end }
    }
end
                        ...
                        if it.bindKey and (it.type=="toggle" or it.type=="action") then
                            if Menu.IsKeyJustPressed(it.bindKey) then
                                if it.type=="toggle" then
                                    ...
                                    if it.setting == "EditorMode" or it.name == "Modo editor" then Menu.EditorMode = it.value end
                                    if it.setting == "ShowKeybinds" or it.name == "Mostrar teclas rápidas" then Menu.ShowKeybinds = it.value end
                                    if it.setting == "ShowSnowflakes" or it.name == "Copos de nieve" then Menu.Settings.ShowSnowflakes = it.value end
